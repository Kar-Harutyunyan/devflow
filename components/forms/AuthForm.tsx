            ...
            <form onSubmit={form.handleSubmit(handleSubmit)} className="mt-10 space-y-6">
                    ...
                    <FormField
                        ...
                        render={({field}) => (
                            <FormItem className="flex w-full flex-col gap-2.5">
                                <FormLabel
                                    className="paragraph-medium text-dark400_light700">{field.name === 'email' ? "Email Address" : field.name.charAt(0).toUpperCase() + field.name.slice(1)}</FormLabel>
                                <FormControl>
                ...
                <Button disabled={form.formState.isSubmitting}
                    ...
                    {form.formState.isSubmitting ?
                        buttonText === 'Sign In' ? "Sign In..." : "Signing Up..."
                        :
                        buttonText
                    }
                </Button>
                ...
                {formType === "SIGN_IN" ? (
                    <p>
                        Don't have an account?{" "} <Link className="paragraph-semibold primary-text-gradient"
